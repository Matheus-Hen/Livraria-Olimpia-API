...
import EstoqueValidacao from '../services/validacaoEstoque.js';
...
const estoqueController = {
  ...
  InsereEstoque: async (req, res) => {
    const body = req.body;
    try {
      const resposta = await estoqueModel.inserirEstoque(
        body.produto,
        body.quantidade,
        body.fornecedor,
      );
      ...
      res.status(resposta.status).json({ msg: error.message, erro: true });
    }
  },
  ...
  atualizaEstoque: async (req, res) => {
    ...
    try {
      const resposta = await estoqueModel.atualizarEstoque(
        id,
        body.produto,
        body.quantidade,
        body.fornecedor,
      ...
      res.status(resposta.status).json({ msg: error.mensagem, erro: true });
    }
  },
